...
| `interceptors` | `moon run src/main/interceptors` | 请求拦截器补 `Authorization`、请求 LIFO / 响应 FIFO 的顺序、响应拦截器改写 JSON 与响应头、请求拦截器中止 + **请求侧**错误处理器救回（错误按来源分流）、`stream` 只走请求侧链、具名注册与按名撤下（同名替换原位） | 无（全部打本机 echo server） |
