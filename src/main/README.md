...
| `interceptors` | `moon run src/main/interceptors` | 请求拦截器补 `Authorization`、请求 LIFO / 响应 FIFO 的顺序、响应拦截器改写 JSON 与响应头、请求拦截器中止 + `on_rejected`、`stream` 只走请求侧 | 无（全部打本机 echo server） |
