...
4. **加自动重连**：那是一个新能力，不是解析规则的一部分；它需要决定「何时放弃重试」「如何结束一条流」，应当先补一节设计再动手。**结束一条已经在消费的流已经有两条路**：`SseStream::close()`（就到此为止），或者 `cancel_token` 取消（`next_event` 会抛 `ERR_CANCELED` 而不是返回 `None`，见 `12-cancellation.md`）——别把「取消」实现成返回 `None`，那会和「服务端正常结束事件流」混在一起。
