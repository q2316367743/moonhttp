...
| `pub(all) struct` 里允许个别字段标 `priv` | 三个门面类型都靠这条：`StreamResponse` / `SseStream` 的响应体流私有（读取必须走本类型的方法，错误才能统一成 `HttpError`）、`Response` 的 `raw` 私有（读法收敛到 `bytes()` / `text()` / `json()` 三个入口，解码规则只有一个落点）；`Config.data` 同理（请求体只能经四个构建器设置） |
