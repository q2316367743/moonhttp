...
| 03 | [请求管线](03-request-pipeline.md) | 三种读法（`request` / `stream` / `sse`）的分工、八个步骤、URL 拼接与 query 序列化规则、请求体三种形态与自动补头、`response_encoding` 如何决定 `data`（以及为什么不做 JSON 解析）、状态码校验 | 要改请求行为（URL、头的优先级、body 处理、响应解码）时 |
