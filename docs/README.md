...
7. **改响应解码（`response_encoding` / `decode_body`）**：四种编码的分派在 `client.mbt` 的 `decode_body`，用例在 `src/encoding_test.mbt`。三条别动的前提：解码一律 lossy（非法字节 → `U+FFFD`，不抛错）、`Response::data` 是原文**不做 JSON 解析**（要 JSON 请调用方自己 `@json.parse`）、这个字段只作用于「读全量」的 `Client::request`。改动要同步 `03-request-pipeline.md` 的表格与 `README.mbt.md`。
