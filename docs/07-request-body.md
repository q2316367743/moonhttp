...
`Config` 的请求体是**私有字段**，只能经三个构建器设置；字节由 `Config::serialize_body()` 产出。
本篇记录这条链路、三种形态的线上格式，以及为什么这么设计。

## 关键文件

| 文件 | 职责 |
|---|---|
| `src/config/body.mbt` | 私有枚举 `Body`（三种形态）、`SerializedBody`、三个 `with_data_from_*` 构建器、`serialize_body` |
...
| `src/config/body_test.mbt` | 字节级用例（三种形态 + multipart 布局） |
| `src/request_test.mbt` | 端到端用例（真正写到连接上的头与 body） |

## 为什么 `data` 是私有字段

老实现里 `data : Json?` 是公开字段，`with_data(Json)` 一个入口承担了所有情况，
靠「`Json::String` 原样发送、其它值 `stringify`」这条隐含规则区分。它有两个问题：
...
所以请求体改成「数据 + 形态标签」的私有枚举 `Body`，三个构建器各管一种形态，
...
（例如手拼的 urlencoded 表单，自己加一条 `Content-Type` 即可）。
...
想彻底自己控制（例如 urlencoded、protobuf），用 `with_data_from_str` 发字节 + 自己设头。
...
| `urlencoded` | 传 `URLSearchParams` 自动编码 | 不内置：手拼字符串 + `with_data_from_str` + 自己设头 |
| `name` / `filename` 转义 | 不转义（node 实现） | 按 WHATWG 转义（防解析错位与注入） |
| `transformRequest` | 可插拔 | 固定为上述三种形态，不做转换器 |
